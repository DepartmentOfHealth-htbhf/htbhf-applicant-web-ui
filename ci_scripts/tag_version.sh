...
# If PACKAGE_VERSION matches the first part of GIT_VERSION (major.minor.), just increment the patch version, otherwise use major.minor from package.json and reset patch to 0
NEW_VERSION=$(echo "$PACKAGE_VERSION $GIT_VERSION" | awk '{printf($1==$2?$2$3+1:$1"0")}')
